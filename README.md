...
| `COMPOSIO_API_KEY`       | ✅ Required | Composio API key (get one at composio.dev) — powers the integrations UI    |
| `OPENCLAW_GATEWAY_TOKEN` | 🔒 Auto     | Auto-generated by Render                                                    |
| `PORT`                   | 🔒 Auto     | Set by Render                                                               |
| `WEBHOOK_TOKEN`          | 🔒 Auto     | Auto-generated by Render                                                    |

> **Note on this fork:** the DenchClaw integrations UI in this template is
> wired to call Composio's REST API directly with your own `COMPOSIO_API_KEY`
> instead of routing through the Dench Cloud gateway. Connect-app, list, and
> disconnect flows all hit `https://backend.composio.dev/api/v3.1/*`.
> Agent-side tool execution (the `dench_*` MCP tools) still uses the Dench
> gateway — a future phase will swap that too.
